...
n = int(input("Number of ways : "))
numoflines = int((2**17)/(n))         #assuming that the cache block size is 4 bytes
cc = [[0 for i in range(n)] for j in range(numoflines)]
bitsofindex = int(math.log(numoflines,2))
bitsoftag = 30 - bitsofindex
...
class cache():
    ...
    def __init__(self,data):
        ...
        self.strindex = self.hb[bitsoftag:30]          #strindex is index in str form
    ...
    def logic(self):
        ...
        for k in range(0,n):
            if(self.tag == cc[self.decindex][k]):
                self.out = 1
                return self.out

        for l in range(0,n):
            if(cc[self.decindex][l] == 0):
                cc[self.decindex][l] = self.tag
                self.out = 0
                return self.out
